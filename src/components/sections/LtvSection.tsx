...
const LtvSection = () => {
  ...
  return (
    <section id="ltv" ref={ref} style={{ background: "#003035", color: "#F8F7F3", padding: "120px 60px" }}>
      ...
      {/* LTV Grid */}
      <div className="reveal" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: "rgba(255,255,255,0.06)", marginBottom: 2 }}>
        {ltvCards.map((card) => (
          <div key={card.label} style={{ background: "rgba(255,255,255,0.03)", padding: 48, position: "relative", overflow: "hidden" }}>
      ...
      {/* Total bar */}
      <div className="reveal" style={{ background: "rgba(255,220,90,0.06)", border: "1px solid rgba(255,220,90,0.15)", padding: "36px 60px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 60 }}>
        <div>
      ...
      {/* Cross-sell EPSA */}
      <div className="reveal" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, background: "rgba(255,255,255,0.06)", marginBottom: 60 }}>
        {crossSells.map((item) => (
